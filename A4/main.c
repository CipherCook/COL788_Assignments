...
void heart_rate_measurement(int current_value_red, int current_value_ir, char* uartBuf, FIL* fil) {
...
//  f_mount(NULL, "", 0);
  ...
  //			uint32_t start_time = HAL_GetTick();
		  ...
		  if (finger_detected) {
			  heart_rate_measurement(current_value_red, current_value_ir, uartBuf, &fil);
		  }
	  }
  }
  /* USER CODE END 3 */
